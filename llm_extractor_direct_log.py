...
import state_transition_visualizer as stv
...
def load_dotenv(env_file_path: Path) -> None:
    """.env の KEY=VALUE を読み込み、未設定の環境変数に反映する。"""
...
def parse_pattern_records(response_text: str) -> List[dict]:
    """LLM応答をパターン配列に正規化して返す。"""
    # 1) 応答全体とコードブロック(JSON)の両方を候補として試す
...
def call_gemini(
    ...
    """Geminiを呼び出し、応答テキストと利用SDK名、トークン使用量、応答時間を返す。"""
    try:
        import importlib
        ...
        try:
            ...
            # まれにHTTPクライアントが閉じた状態になるため、1回だけ再試行する
        ...
        if text: # 新SDKの応答形式でテキストが直接返される場合はそれを使用
            return text, "google-genai", usage, duration_sec

        # 新SDKの応答形式でテキストが直接返されない場合、parts を結合してテキストを構築
        parts = []
...
def filter_days(df: pd.DataFrame, days: int) -> pd.DataFrame:
    ...
    return df[df["timestamp"] < end].copy()
