...
確認日: 2026-09-09。各節の「文書→入口/実装→テスト」を使う。評価変更前は [既知の不一致](../known_issues.md) の該当IDも確認。テストは関連候補であり評価全体の保証ではない。
該当節のみ取得: `rg -n -A 6 '^## eval6 ' docs/codex_memory/EVALUATION_ROUTES.md`（番号を置換）。実行条件は仕様docsと実際のCLIで確認する。

## eval1 旧K/h感度分析

- 文書: [評価1](../evaluation_1_parameter_sensitivity.md)。現行論文の感度評価はeval7。
...
- 文書: [評価2](../evaluation_2_proposed_method_5runs.md)。
...
- 文書: [評価3](../evaluation_3_direct_log_baseline_comparison.md)。
...
- 文書: [評価4](../evaluation_4_labeled_casas_adl.md)。
...
- 文書: [評価5](../evaluation_5_adl_correspondence.md)。
...
- 文書: [評価6](../evaluation_6_adl_interpretation_set.md)。
...
- 文書: [評価7](../evaluation_7_parameter_sensitivity_adl_interpretation.md)。
...
- 文書: [評価8](../evaluation_8_frequency_stratified_adl_consistency.md)。
