...
| 評価5 | frequency / rule-filtered / FP-Growth / proposedをパターン単位で比較する。 |
...
「ステップ実行」タブには、不足ファイル生成用の一括実行ボタンがあります。このボタンは評価本体や比較本体を実行せず、前段ステップのうち期待出力が不足しているコマンドだけを上から順に実行します。評価本体は、コマンドと入力ファイルを確認したうえで個別ボタンから実行します。
...
1. 評価5用 `state_series.csv` を作成  
   評価4の `scripts/evaluate_adl_labels.py --write-state-series` を使います。既に `results/4_adl_evaluation/state_series.csv` がある場合はスキップできます。
...
変更可能な主な引数は、train/test split、grounded/useless閾値、assigned ADL閾値、FP-Growth設定、Other状態・Other ADLの扱いです。
...
Streamlit画面では、`代表状態数 K` と `ハミング距離閾値` を `10,15,20,30` のようにカンマ区切りまたは空白区切りで指定できます。`不足ファイル作成ステップを表示` をONにすると、各条件ごとの前段3ステップが不足ファイル生成の一括実行対象になります。一括実行では、既に出力が揃っている前段ステップを対象外にし、不足している条件別ファイルを作ります。評価7本体は一括実行では走らないため、入力が揃ったことを確認してから個別ボタンで実行します。結果タブでは条件別summaryと `evaluation7_summary.json` の `best_condition` を確認できます。
...
各ステップのログはrun名付きのサブディレクトリに保存されます。一括実行では `{run名}_batch` のログディレクトリに、不足ファイル生成として実行したコマンドだけを番号付きログで保存します。コマンド履歴は `output/logs/evaluation_dashboard/command_history.jsonl` に追記されます。
...
| 評価6でrunが足りない | `--runs` と実在するJSON数を揃えるか、`skip-missing-runs` を有効化する。 |
