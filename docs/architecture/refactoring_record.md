...
- [known_issues.md](../research/known_issues.md) の未確定挙動を、リファクタリング時に暗黙の仕様へしない。
...
4. CLIごとに固定された入力・出力パスと設定ファイルの適用範囲を棚卸しする。方針決定までは [KI-14](../research/known_issues.md) として扱う。
5. LLM promptの外部版とfallbackの差を研究条件として解消する。現在は [KI-12](../research/known_issues.md) のため文言だけを同期しない。
...
これらを変更する作業では、対応する評価文書、[artifact_policy.md](../operations/artifact_policy.md)、[known_issues.md](../research/known_issues.md) を先に確認する。
