...
| 評価全体の入口と順序 | [experiment_reproduction.md](../operations/experiment_reproduction.md) |
| 論文採用パラメータ | [paper_parameters.md](../research/paper_parameters.md) |
| 評価固有のCLI・入出力・指標 | `evaluation_1_*.md` から `evaluation_8_*.md` |
| 成果物の扱い | [artifact_policy.md](../operations/artifact_policy.md) |
| 未確定の実装・文書差 | [known_issues.md](../research/known_issues.md) |
| 段階的な構造整理の記録 | [refactoring_record.md](refactoring_record.md) |

## 共通設定の注意

`experiment_config.py` は `configs/default.yaml` を読み、既存モジュールへ定数を公開する互換レイヤーである。ただし、すべてのCLI引数や入出力パスを設定ファイルが支配するわけではない。共通既定の `K=15, h=1, 154日` と、論文採用条件の `K=15, h=0` を区別し、対象評価のCLIと [known_issues.md](../research/known_issues.md) を確認する。
