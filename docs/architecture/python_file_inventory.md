...
| `scripts/run_direct_log_baseline.py` | ラベル付きCASAS txtからactivity labelを除き、代表状態系列をネットワーク化せず直接LLMへ入力するベースラインを実行・評価する。代表状態への最終写像条件は [known_issues.md](../research/known_issues.md) のKI-04を参照。評価6では `--log-days 14 --extract-only` で14日分のみ抽出し、`--n-states`, `--hamming-threshold`, `--runs` で条件別・複数回分を生成できる。 |
