...
`examples/experiments/noise_free.yaml` は変更可能な本実験の初期案です。
3住宅それぞれについて、基準から1因子だけ変える8条件、計24条件を生成します。
因子間の相互作用を網羅する直積実験ではありません。

| 因子 | 基準 | 変更条件 |
|---|---|---|
| 住宅・設備 | compact: リビングを中心に接続、5空間 | corridor: 廊下を追加、6空間 / branched: 書斎を追加しリビング経由で接続、7空間 |
| 住人数 | 1人 | 2人。2人目の予定は20分後、同一の共有設備を使用 |
| 生活習慣 | early: 朝型 | late: 予定を2時間後へ / home: 外出を在宅作業へ置換 |
| 反復頻度 | daily: 食事3回・休憩2回/日 | low: 食事2回・休憩1回 / high: 食事4回・休憩3回 |
| 行動の揺らぎ | small: 開始±10分、活動・step時間±10% | fixed: 揺らぎ0 / large: 開始±45分、活動・step時間±30% |
| 観測ノイズ | 無効 | 変更しない。欠損・遅延・誤反応等を追加しない |

空間数には屋外を含みます。屋外に人感センサーは置かず、出入口のドアを観測します。
全住宅で部屋人感・照明・ドア・コーヒー機器・TV・PC用プラグを使用します。
住宅条件は間取りとセンサー数・配置の複合条件であり、それぞれの単独効果は分離しません。

通常の作業は1回/日、homeのみ追加1回/日です。食事・休憩・作業には共通の
...
本実験の初期案は24条件 × 3seed × 14日（前半7日/後半7日）、LLMは各ログ3反復です。
全4時間帯がある場合、新規のモード抽出は864回、さらにparse/通信retryがあり得ます。
