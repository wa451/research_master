...
uv run python scripts/run_llm_extraction.py --hamming-threshold 0
```
...
uv run python scripts/evaluate_adl_labels.py \
  ...
  --sensor-map configs/aruba_sensor_map.json \
  --patterns output/aruba_15_0_154days/llm_sequences_modes_15_0_154days_1.json \
...
評価6の結果は、指定したroot直下の条件別ディレクトリに保存されます。採用条件では `results/6_adl_match/15_0_14days/` です。

5回分の平均を出す場合は、提案手法JSONと前処理済み代表状態系列を直接LLMへ入力するベースラインJSONをそれぞれ5回分用意し、最後の評価6比較コマンドに `--runs 5` を追加します。生成コマンドは `docs/evaluation_6_adl_interpretation_set.md` を参照してください。
...
- `scripts/run_direct_log_baseline.py`: ラベル付きCASAS txt由来のセンサーイベントに提案手法と同じ前処理・代表状態写像を適用し、代表状態系列を直接LLMへ入力するベースラインを実行。`--n-states`, `--hamming-threshold`, `--runs 5` で条件別・複数回の出力を生成。
...
- `docs/evaluation_7_parameter_sensitivity_adl_interpretation.md`: 全28条件の初回探索と上位10条件・合計5試行によるK・ハミング距離感度評価。
- `docs/evaluation_8_frequency_stratified_adl_consistency.md`: own pattern IDの一意な出現を用いた頻度帯別ADL整合性評価。
