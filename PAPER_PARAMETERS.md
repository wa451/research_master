...
### 1.1 状態遷移ネットワーク構築 (`state_transition_visualizer.py`)
...
| **遅延OFF窓幅** | 180秒 | チャタリング除去時のスムージング窓（秒） |
...
#### 確率的閾値抽出 (`prob_threshold_extractor_modes.py`)
...
### 1.3 LLM抽出手法 (`llm_extractor_modes.py`)
...
- 出力制約: 
  - シーケンス長: 2〜4ノード
  - 形式: 「パターン名」「解釈の根拠」「遷移のシーケンス」を含むJSON
  - 行動ルートの網羅性: 遷移確率20%以上のエッジをたどるすべてのパターンを抽出

---

### 1.4 評価指標 (`evaluate_metrics.py`)
...
1. **状態遷移ネットワーク構築:**
   - 代表状態K=15（頻度上位）
   - ハミング距離閾値=1
   - サンプリング間隔1秒、遅延OFF窓幅180秒によるセンサーノイズ除去
